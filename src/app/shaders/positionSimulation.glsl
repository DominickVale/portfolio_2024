...
//     vec4 tmpPos = texture2D( uPositionTexture, uv ); // get current positions from texture 
...
uniform float uRho;
uniform float uBeta;
const float dt = 0.001; // Adjust time step size as needed
...
void main() {
    ...
    // RK4 integration
    vec3 k1 = dt * lorenz(position);
    vec3 k2 = dt * lorenz(position + 0.5 * k1);
    vec3 k3 = dt * lorenz(position + 0.5 * k2);
    vec3 k4 = dt * lorenz(position + k3);
